...
void Window::Camera::setClamp(const int windowW, const int windowH) {
	if (center.x - radiusWidth < 0) { center.x = radiusWidth; }//left edge
	if (center.y - radiusHeight < 0) { center.y = radiusHeight; }//top edge
	if (center.x + radiusWidth > windowW) { center.x = windowW - radiusWidth; }//right edge
	if (center.y + radiusHeight > windowH) { center.y = windowH - radiusHeight; }//bottom edge
}
const SDL_FPoint Window::Camera::getCenter()const {
	return center;
}
